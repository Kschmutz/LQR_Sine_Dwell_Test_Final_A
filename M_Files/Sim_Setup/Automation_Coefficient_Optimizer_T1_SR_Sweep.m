...
for cntr=1:sim_pts
    %for cntr1=1:sim_pts_f_SR
        ...
        Slip_Err_P_Gain = 10^(-7.25 + cntr/4)
        Tau_SRC = 1/(2*pi*250)
...
%        f_SR_Saved(cntr1) = 1/(2*pi*Tau_SRC)
        ...
        FL_rms_err(cntr) = rms(VMC(tstart:find(VMC(:,16)<0.01,1),11) - -0.1)
        FR_rms_err(cntr) = rms(VMC(tstart:find(VMC(:,16)<0.01,1),12) - -0.1)
        RL_rms_err(cntr) = rms(VMC(tstart:find(VMC(:,16)<0.01,1),13) - -0.1)
        RR_rms_err(cntr) = rms(VMC(tstart:find(VMC(:,16)<0.01,1),14) - -0.1)
...
semilogx(ax11,Slip_Err_Gain_Saved,FL_rms_err)
ax12 = subplot(5,1,2); % bottom subplot
semilogx(ax12,Slip_Err_Gain_Saved,FR_rms_err)
ax13 = subplot(5,1,3); % bottom subplot
semilogx(ax13,Slip_Err_Gain_Saved,RL_rms_err)
ax14 = subplot(5,1,4); % bottom subplot
semilogx(ax14,Slip_Err_Gain_Saved,RR_rms_err)
ax15 = subplot(5,1,5); % bottom subplot
semilogx(ax15,Slip_Err_Gain_Saved,tot_rms_err)
hold on

axis([ax11],[1e-7 1e10 0 1.5]);
grid(ax11,'on');
ax11.XMinorGrid = 'on';
ax11.YMinorGrid = 'on';

axis([ax12],[1e-7 1e10 0 1.5]);
grid(ax12,'on');
ax12.XMinorGrid = 'on';
ax12.YMinorGrid = 'on';

axis([ax13],[1e-7 1e10 0 1.5]);
grid(ax13,'on');
ax13.XMinorGrid = 'on';
ax13.YMinorGrid = 'on';

axis([ax14],[1e-7 1e10 0 1.5]);
grid(ax14,'on');
ax14.XMinorGrid = 'on';
ax14.YMinorGrid = 'on';

axis([ax15],[1e-7 1e10 0 6]);
grid(ax15,'on');
ax15.XMinorGrid = 'on';
ax15.YMinorGrid = 'on';

xlabel(ax15,'P_{e_{\lambda}}','FontSize',16);
...
%save rms error plot
Filename_fig_rms_err = sprintf('Sliding_Mode_Control_ABS_Test_rms_err_fig_175Nm_T1_%s.fig', datestr(now,'mm-dd-yyyy_HH-MM'));
savefig(Filename_fig_rms_err);

%save workspace to file
Filename_ABS = sprintf('Sliding_Mode_Control_ABS_Test_ABS_Test_175Nm_T1_%s.mat', datestr(now,'mm-dd-yyyy_HH-MM'));
save(Filename_ABS);
