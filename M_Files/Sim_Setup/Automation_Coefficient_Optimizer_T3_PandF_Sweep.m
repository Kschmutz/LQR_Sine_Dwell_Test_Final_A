...
for cntr=1:sim_pts
    for cntr1=1:sim_pts_f_SR
        ...
        Slip_Err_P_Gain = cntr*10000 - 5000
        Tau_SRC = 1/(2*pi*(3 + cntr1))
...
% %         FL_rms_err(cntr1) = rms(VMC(tstart:find(VMC(:,16)<0.01,1),11) - -0.1)
% %         FR_rms_err(cntr1) = rms(VMC(tstart:find(VMC(:,16)<0.01,1),12) - -0.1)
% %         RL_rms_err(cntr1) = rms(VMC(tstart:find(VMC(:,16)<0.01,1),13) - -0.1)
% %         RR_rms_err(cntr1) = rms(VMC(tstart:find(VMC(:,16)<0.01,1),14) - -0.1)
% %         
        %analyze data and make decision
        min_new = find(VMC(:,16)<0.01,1);          %Check min Vx
        if  VMC(2000,16) < 16 ...               %make sure Vx is lower than 12m/s by 3s
            ...
            for num = 1:(nsp-1)
                if min_new < Vx_arr(num);
                    for s = num:nsp
                        ...
                        fc_SR_Lowest_X(1,s+1) = fc_SR_Lowest(1,s);
                    ...
                    fc_SR_Lowest = fc_SR_Lowest_X;
                    %Lat_Accel_Err_Gain_Lowest = Lat_Accel_Err_Gain_Lowest_X;
                    
                    Gains(1,:) = Slip_Err_P_Gain_Lowest_X; 
                    Gains(2,:) = fc_SR_Lowest_X;
...
% % figure % new figure
% % hold on
% % ax11 = subplot(5,1,1); % top subplot
% % semilogx(ax11,f_SR_Saved,FL_rms_err)
% % ax12 = subplot(5,1,2); % bottom subplot
% % semilogx(ax12,f_SR_Saved,FR_rms_err)
% % ax13 = subplot(5,1,3); % bottom subplot
% % semilogx(ax13,f_SR_Saved,RL_rms_err)
% % ax14 = subplot(5,1,4); % bottom subplot
% % semilogx(ax14,f_SR_Saved,RR_rms_err)
% % ax15 = subplot(5,1,5); % bottom subplot
% % semilogx(ax15,f_SR_Saved,tot_rms_err)
% % hold on
% % 
% % axis([ax11],[0.01 100 0 1]);
% % grid(ax11,'on');
% % ax11.XMinorGrid = 'on';
% % ax11.YMinorGrid = 'on';
% % 
% % axis([ax12],[0.01 100 0 1]);
% % grid(ax12,'on');
% % ax12.XMinorGrid = 'on';
% % ax12.YMinorGrid = 'on';
% % 
% % axis([ax13],[0.01 100 0 1]);
% % grid(ax13,'on');
% % ax13.XMinorGrid = 'on';
% % ax13.YMinorGrid = 'on';
% % 
% % axis([ax14],[0.01 100 0 1]);
% % grid(ax14,'on');
% % ax14.XMinorGrid = 'on';
% % ax14.YMinorGrid = 'on';
% % 
% % axis([ax15],[0.01 100 0 2]);
% % grid(ax15,'on');
% % ax15.XMinorGrid = 'on';
% % ax15.YMinorGrid = 'on';
% % 
% % xlabel(ax15,'fc_{e_{\lambda}}','FontSize',16);
% % 
% % ylabel(ax11,'rms(e_{\lambda\_FL})','FontSize',30);
% % ylabel(ax12,'rms(e_{\lambda\_FR})','FontSize',30);
% % ylabel(ax13,'rms(e_{\lambda\_RL})','FontSize',30);
% % ylabel(ax14,'rms(e_{\lambda\_RR})','FontSize',30);
% % ylabel(ax15,'tot rms(e_{\lambda})','FontSize',30);
% % 
% % set(ax11,'FontSize',12);
% % set(ax12,'FontSize',12);
% % set(ax13,'FontSize',12);
% % set(ax14,'FontSize',12);
% % set(ax15,'FontSize',12);
% % 
% % %save rms error plot
% % Filename_fig_rms_err = sprintf('Sliding_Mode_Control_ABS_Test_rms_err_fig_175Nm_T2_%s.fig', datestr(now,'mm-dd-yyyy_HH-MM'));
% % savefig(Filename_fig_rms_err);

%save workspace to file
Filename_ABS = sprintf('Sliding_Mode_Control_ABS_Test_ABS_Test_175Nm_T3_%s.mat', datestr(now,'mm-dd-yyyy_HH-MM'));
save(Filename_ABS);
...
Filename_fig = sprintf('Sliding_Mode_Control_ABS_Test_fig_175Nm_T3_%s.fig', datestr(now,'mm-dd-yyyy_HH-MM'));
